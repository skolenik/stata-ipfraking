...
affixed to the data set with \stcmd{svyset} command; see \svyref{svyset}).
...
Suppose there are several variables, referred to
...
If \stcmd{ipfraking} determines that the categories do not match
...
weight variables that can be used in production diagnostics. 
...
One of these utility programs, \stcmd{mat2do},
...
\stcmd{whatsdeff} and \stcmd{totalmatrices}.
...
of the official Stata \stcmd{svy} suite available in Stata 15.1+ \citep{valliant:dever:2017}).
...
the formats expected by \stcmd{ipfraking} and \stcmd{svycal} \citep{valliant:dever:2017}.
...
Along the way, \stcmd{ipfraking\_report} runs a regression of the log raking adjustment ratio
...
Functionality of \stcmd{ipfraking\_report} is aimed at manual quality control,
...
is to have cells of size at least 30 to 50.)
...
\hangpara
\stcmd{max(\num)} limits the values of the collapsed variable that can be used in a collapsing rule
...
(See page \pageref{page:mismatch} for the output \stcmd{ipfraking} provides in this situation.)
...
\textit{Hint 2}. The behavior of \stcmd{wgtcellcollapse collapse, zeroes()} 
...
\stcmd{wgtcellcollapse collapse} look for a rule that combines as many cells as possible, 
...
\}
\end{stlog}
while the content of the the \stcmd{r(cfailed)} macro can be used in code that could read
\begin{stlog}
...
Suppose turnstile counts were collected at entrances (\stcmd{board\_id}) and exits (\stcmd{alight\_id}) 
...
The \stcmd{zeroes()} option of \stcmd{wgtcellcollapse collapse} 
...
The \stcmd{maxcategory(99)} option
...
The use of the subsampling conditions like \stcmd{if daypart==5 & inrange(alight\_id,1,36)}
in calls 5 and 6
...
The special missing value \stcmd{.i} that appears in the smallest count report,
...
The use of \stcmd{greedy} option in call 3 made it possible to collapse the streak
...
It prevents \stcmd{wgtcellcollapse} from picking up rules that
...
is welcome to \stcmd{type} them; they contain long sequences of \stcmd{replace}
...
consistent matrices of the population control totals for \stcmd{ipfraking}.
...
using \stcmd{from()} option of \stcmd{ipfraking}.
...
\stcmd{survey} \citep{lumley:2010,lumley:2018:svy334},
...
Second, \stcmd{ipfraking} produces detailed diagnostics of problems and oddities
...
or explicit syntax \stcmd{value.variable = }{\it \#} specification
of \stcmd{svycal}.
...
of using factor variables in regression models (see \uref{Factor variables}). 
It creates the necessary interactions internally on the fly, 
while \stcmd{ipfraking} requires explicit generation of interaction variables.

Finally, of all the Stata weight calibration packages, \stcmd{ipfraking} is unique
