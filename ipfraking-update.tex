...
\stcmd{mat2do} and \stcmd{xls2row}. An additional utility program \stcmd{whatsdeff}
...
${\rm DEFF_{UWE}}=1 + {CV}^2_w = $ \stcmd{1 + r(Var)/(r(mean))\^{}2} from \stcmd{summarize} {\it weight{\_}variable}.
...
Note how \stcmd{break} option of \stcmd{wgtcellcollapse report} can be used to abort the execution
...
of \stcmd{dpston==1000001} indicates \stcmd{daypart==1} and station ID 1.
...
The next improvement I am going to demonstrate is to fix the problem of weak performance in
collapsing the zero cells with an additional option \stcmd{greedy}.
It modifies behavior of \stcmd{wgtcellcollapse} to require that,
...
\begin{figure}[h!]
    \begin{center}
    \epsfig{file=raked_linear,width=400}
    \end{center}
